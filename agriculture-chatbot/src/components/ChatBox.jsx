...
const ChatBox = () => {
  ...
  const handleSend = async () => {
    ...
    try {
      const res = await axios.post("https://agriculture-chatbot-gmqi.onrender.com/api/query", {
        query: input,
      });

      // Format the response with sources
      let responseText = res.data.answer ?? "No answer returned.";
  ...
  return (
    <div className="mx-4 my-6 flex justify-center">
      {/* Container: responsive width and height */}
      <div className="bg-gray-800 rounded-2xl w-full max-w-xl md:max-w-3xl h-[70vh] md:h-[80vh] flex flex-col shadow-lg border border-gray-700">
        {/* Header */}
        <div className="bg-gray-900 p-3 md:p-4 rounded-t-2xl border-b border-gray-700">
        ...
        {/* Example Queries (shown only when initial bot message present) */}
        {messages.length === 1 && (
        ...
        {/* Input Area */}
        <div className="p-3 md:p-4 border-t border-gray-700 flex items-center gap-3">
          <textarea
            className="flex-1 bg-gray-700 p-2 md:p-3 rounded-lg text-white focus:outline-none resize-none text-sm md:text-base max-h-40"
          ...
          />
          <button
            className={`bg-teal-500 hover:bg-teal-600 px-3 md:px-5 py-2 md:py-3 rounded-lg flex items-center justify-center ${
